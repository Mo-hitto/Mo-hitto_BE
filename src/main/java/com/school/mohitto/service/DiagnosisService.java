...
public class DiagnosisService {
    ...
    @Transactional
    public UploadImageResponse uploadImage(Long diagnosisId, MultipartFile image) {
        Diagnosis diagnosis = diagnosisRepository.findById(diagnosisId)
                .orElseThrow(() -> new CustomException(ErrorCode.DIAGNOSIS_NOT_FOUND));

        if (image.isEmpty()) throw new CustomException(ErrorCode.INVALID_PARAMETER);

        String imageUrl = fileUploader.upload(image, "diagnosis"); // local or S3
        UploadImage uploadImage = UploadImage.of(diagnosis, imageUrl);
        uploadImageRepository.save(uploadImage);

        return new UploadImageResponse(uploadImage.getId(), imageUrl);
    }
