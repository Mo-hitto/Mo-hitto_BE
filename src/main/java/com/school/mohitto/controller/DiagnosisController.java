...
public class DiagnosisController {
    ...
    @PostMapping("/{diagnosisId}/upload")
    @Operation(
            summary = "진단 이미지 업로드",
            description = "진단 결과 생성을 위한 얼굴 이미지를 업로드합니다.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "업로드 성공",
                            content = @Content(schema = @Schema(implementation = UploadImageResponse.class))),
                    @ApiResponse(responseCode = "400", description = "파일 누락 또는 형식 오류",
                            content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
            }
    )
    public UploadImageResponse uploadDiagnosisImage(
            @PathVariable Long diagnosisId,
            @RequestPart("image") MultipartFile image
    ) {
        return diagnosisService.uploadImage(diagnosisId, image);
    }
